...
# Amalgamating several modules together for use in a jupyter notebook
...
'''
The following functions are directory management utils
'''
...
''' 
The following functions are for dealing with dataset overlap
'''
...
'''
The following classes and functions deal with handling the datasets and model training.
Note that Boneage and Arxiv have different training functions, and they are supplied in the relevant sections.
Census training is merely clf.fit
'''

######################Common filtering and heuristic functions#####################
...
class BinaryNet(torch.nn.Module):
    def __init__(self,num_features):
        ...
        self.fc1 = torch.nn.Sequential(torch.nn.Linear(num_features, 1024),nn.Tanh(),)
        self.fc2 = torch.nn.Sequential(torch.nn.Linear(1024, 512),nn.Tanh(),)
        self.fc3 = torch.nn.Sequential(torch.nn.Linear(512, 256),nn.Tanh(),)
        self.fc4 = torch.nn.Sequential(torch.nn.Linear(256, 128),nn.Tanh(),)
...
def load_model_census(path, num_features):
    model = BinaryNet(num_features=num_features)
...
def get_model(num_features):
    clf = BinaryNet(num_features=num_features)
    return clf
...
def boneage_train(model, loaders, lr=1e-3, epoch_num=10,weight_decay=0, get_best=False):
    ...
    for epoch in iterator:
        _, tacc = boneage_train_epoch(train_loader, model,criterion, optimizer, epoch)
        vloss, vacc = boneage_validate_epoch(val_loader, model, criterion)
...
def boneage_train_epoch(train_loader, model, criterion, optimizer, epoch):
    model.train()
    ...
    for data in iterator:
        ...
        try:
            outputs_altered = outputs[:, 0]
            loss = criterion(outputs_altered, labels.float())
        except:
            # Case of torch census
...
def boneage_validate_epoch(val_loader, model, criterion, verbose = True, return_preds = False):
    model.eval()
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            outputs = model(images)
            try:
                outputs_alter = outputs[:, 0]
                prediction = (outputs_alter >= 0)
                if return_preds:
                    preds_to_ret.append(prediction)
                val_acc.update(prediction.eq(labels.view_as(prediction)).sum().item()/N)
                val_loss.update(criterion(outputs_alter, labels.float()).item())
            except:
                prediction = (outputs >= 0)
                if return_preds:
                    preds_to_ret.append(prediction)
